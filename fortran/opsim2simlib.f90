SUBROUTINE LSSTSIMLIB(OPSIMF, SIMLIB) 
! Take LSST opsim output and produce sdss simlib input
IMPLICIT NONE
!
character (len = 500):: LINE
!
integer :: OID,SID,FID,PID,SNO,EDATE,ETIME,FINT,FFINT, PAIRN,NIGHT
double precision ::  SLEWT,SLEWD,ROTS,ROTT,PRANK,FRANK,MSEE,RSEE,SEE
double precision ::  XPAR,CSEE,AMASS,SKYBV,VSKYB,RA,DEC,LST,ALT,AZI 
double precision :: MOOND,MOONRA,MOONDEC,MOONALT,MOONPHA, SUNA,SUNAZ,PHANG
double precision :: RSCAT,MIESCAT,MOONIL,MOONB,DARKB, F5SIG,PSKYB,P5SIG,MSKYB
double precision :: M5SIG,HEXDIRA,HEXDIDEC,FVIS, VERTEX,SOLEL
double precision :: MJD,MJDL

CHARACTER (len=1) :: FILT
CHARACTER (len =7) ::SUBQ
! These in OpsimObs, but not Obsim
      DOUBLE PRECISION :: MOONAZ,MOONPHAO
      INTEGER :: DOWN
      CHARACTER (LEN =1) :: MAKM
!
! List of all LSST fields that are covered in the opsim file
!
!      INTEGER :: MXFLD,MXEP,MSURV,MXOFLD,MXOEP
      INTEGER, PARAMETER :: MXFLD=4000
      INTEGER, PARAMETER :: MXEP=2000
      INTEGER, PARAMETER :: MSURV=1
      INTEGER, PARAMETER :: MXOFLD=100
      INTEGER, PARAMETER :: MXOEP=100000
...
!REALLY DOUBLE
      DOUBLE PRECISION :: FMJD(MXFLD,MXEP)
      DOUBLE PRECISION :: FSKYB(MXFLD,MXEP), PSF(MXFLD,MXEP),FSIG5(MXFLD,MXEP)
DOUBLE PRECISION ::  FRA(MXFLD),FDEC(MXFLD), FMWEBV(MXFLD),FPIXS(MXFLD)
...
!REALLY DOUBLE
      DOUBLE PRECISION:: FMJDO(MXOFLD,MXOEP)
      DOUBLE PRECISION:: FSKYBO(MXOFLD,MXOEP), PSFO(MXOFLD,MXOEP),FSIG5O(MXOFLD,MXOEP)
      DOUBLE PRECISION:: FRAO(MXOFLD),FDECO(MXOFLD), FMWEBVO(MXOFLD),FPIXSO(MXOFLD)
      CHARACTER ::FFLTO(MXOFLD,MXOEP)
      INTEGER ::IFLTO(MXOFLD,MXOEP),IORDO(MXOFLD,MXOEP)
!C
      INTEGER ::IFIL,IFILT
      INTEGER, PARAMETER :: NLSSTF=6
...
!C
      CHARACTER (len=100) :: OPSIMF ,SIMLIB 
!C
!C Type of input
!C
      LOGICAL :: OPSIM
...
!C
      DOUBLE PRECISION ::SKYS,PSF1,RAD,DECD
      INTEGER ::COUNT
!C
!C Limitation on how many epoch to write
!C
      INTEGER :: NEPW
      INTEGER, PARAMETER :: MXEPWR=MXEP+MXOEP
      INTEGER :: NTGDW
!C
!C Zeropoint
!C
      DOUBLE PRECiSION ::  ZPT5,F5,D51,D52,D5,N5
!C
!C Sky noise
!C
      DOUBLE PRECISION:: SKYM,SKYF , TSL
      INTEGER :: NNIT,JEP,INIT,IWEP,JEPN,JEPX
!C
!C Paramters for simlib
!C
      DOUBLE PRECISION, PARAMETER ::GAIN=1.0
      DOUBLE PRECISION, PARAMETER ::SNRLM=5.0 
!C
...
!          READ (LINE,*) OID, SID, PID, FID, FILT, SNO, SUBQ, PAIRN, EDATE, MJD, NIGHT, ETIME, SLEWT, SLEWD, ROTS, ROTT, FVIS, FINT, FFINT, PRANK, FRANK,MSEE,RSEE,SEE,XPAR,CSEE,AMASS, SKYBV,VSKYB,RA,DEC,LST,ALT,AZI,MOOND,MOONRA, MOONDEC,MOONALT,MOONPHA,SUNA,SUNAZ,PHANG,RSCAT, MIESCAT,MOONIL,MOONB,DARKB,SOLEL,F5SIG,PSKYB, P5SIG,MSKYB,M5SIG,HEXDIRA,HEXDIDEC,VERTEX
!          DOWN = 0
        ELSEIF (OPSIMOBS) THEN
READ (LINE,*) RA,DEC,MJD,FILT,ALT,AZI,AMASS,XPAR,RSEE,SEE, &
 SUNA,SUNAZ,MOONALT, MOONAZ,MOONPHA,MOONPHAO, MSKYB, M5SIG,NIGHT,DOWN,FID, MAKM
...
!C Make an ID for this from the propID and field ID
          ...
          IF (NEPO(IFL).EQ.MXOEP) THEN
            WRITE (6,*) 'Epoch limit of ',MXOEP, ' for overflow FIELD ',FID
            STOP
          ENDIF
        ENDIF
...
!C Write the header
!C
      WRITE (32,'(A)') 'SURVEY: LSST    FILTERS: ugrizY  TELESCOPE: LSST'
...
!NEED to add statement below
WRITE (32,'(A,F10.6,A,F10.6,A,I5,A,F6.3,A,F6.3,A,I5)') 'RA: ',RAD, &
'    DECL: ',DECD,'   NOBS: ', MIN(NEPW,MXEPWR), '    MWEBV:',  FMWEBV(IFLC),'   PIXSIZE:',FPIXS(IFLC), '    FIELD:',IFLD(IFLC)
        WRITE (32,'(A)') ''
        WRITE (32,'(A1,27X,A)') '#','CCD  CCD        PSF1 PSF2 PSF2/1'
        WRITE (32,'(A,A)') '#    MJD        IDEXPT  FLT GAIN NOISE', ' SKYSIG (pixels)  RATIO  ZPTAVG ZPTSIG  MAG'
        NTGDW = 0
        IF (.NOT.OVFL(IFLC)) THEN
          DO IEP = 1,NEP(IFLC)
            DO JEP = 1,NEP(IFLC)
...
!C Use Kessler routine to compute psf, zpt and skysig
            WRITE (6,'(A)') "ENTERED with wrong numbers "
            CALL OPSIM2SIMLIB(FSIG5(IFLC,IWEP),SNRLM, FSKYB(IFLC,IWEP), PSF(IFLC,IWEP), ZPT5,SKYS,PSF1)
!NEED TO ADD the statement below
WRITE (*,*) NTGDW
WRITE (*,*) MXEPWR
WRITE (*,*) "HELPO"
IF (NTGDW.LE.MXEPWR) THEN
     WRITE (*,*) SKYS, PSF1, ZPT5
     WRITE (32,'(A,F11.5,2X,I9,1X,A,2X,A,F6.2,1X,F5.2,A,F5.2,A)') 'S: ', &
FMJD(IFLC,IWEP),FIDID(IFLC,IWEP), FFLT(IFLC,IWEP), '1.00  0.25 ',SKYS,PSF1, ' 0.00 0.000  ',ZPT5,'  0.005 -99.000'
ENDIF
ENDDO
!C And over flows
        ELSE
          ...
          DO IEP = 1,NEPO(IFL)
            ...
            NTGDW = NTGDW + 1
!C Use Kessler routine to compute psf, zpt and skysig
            WRITE (6,'(A)') "ENTERED with right numbers "
            CALL OPSIM2SIMLIB(FSIG5O(IFL,IWEP),SNRLM, FSKYBO(IFL,IWEP), PSFO(IFL,IWEP),ZPT5,SKYS,PSF1)
	    write (*,'(A,F11.5,2X,I9,X,A,2X,A,X,F6.2,X,F5.2,A, F5.2, A)') 'S: ',FMJDO(IFL,IWEP), FIDIDO(IFL,IWEP), &
	FFLTO(IFL,IWEP), '1.00  0.25 ',SKYS, PSF1,' 0.00 0.000  ',ZPT5,'  0.005 -99.000'

            IF (NTGDW.LE.MXEPWR) THEN
	         write (32,'(A,F11.5,2X,I9,X,A,2X,A,X,F6.2,X,F5.2,A, F5.2, A)') 'S: ',FMJDO(IFL,IWEP), FIDIDO(IFL,IWEP), &
	FFLTO(IFL,IWEP), '1.00  0.25 ',SKYS, PSF1,' 0.00 0.000  ',ZPT5,'  0.005 -99.000'
             ! WRITE (32, '(A,F11.5,2X,I9,1X,A)') 'S: ',FMJDO(IFL,IWEP), FIDIDO(IFL,IWEP), FFLTO(IFL,IWEP), '1.00  0.25 '
              !WRITE (32, '(A,F11.5,2X,I9,1X,A,2X,A,F6.2,1X,F5.2,A,F5.2,A)') 'S: ',FMJDO(IFL,IWEP), FIDIDO(IFL,IWEP), FFLTO(IFL,IWEP), '1.00  0.25 ',SKYS, PSF1,' 0.00 0.000  ',ZPT5,'  0.005 -99.000'
            ENDIF
          ENDDO
        ENDIF
...
!C
      ...
      SUBROUTINE OPSIM2SIMLIB(OPSIM_MAGLIM,  OPSIM_SNR, OPSIM_MAGSKY, OPSIM_SEEING  &! (I)
        ,SIMLIB_ZPTAVG, SIMLIB_SKYSIG, SIMLIB_PSF1    ) ! (O)

      IMPLICIT NONE

! Created Jun 10, 2009 by R.Kessler
! convert LSST OPSIM args into SIMLIB args for the SNANA sim.

! subroutine args
double precision :: OPSIM_MAGLIM , OPSIM_SNR, OPSIM_MAGSKY, OPSIM_SEEING
double precision :: SIMLIB_ZPTAVG, SIMLIB_SKYSIG, SIMLIB_PSF1   


double precision :: SNR_MAGLIM!, PIXSIZE, NPIX_ASEC , TEN
double precision, PARAMETER  :: PIXSIZE    = 0.2   ! pixel size, arcsec
double precision, PARAMETER  :: NPIX_ASEC = 1./(PIXSIZE*PIXSIZE)   ! Npix per arcsec
double precision, PARAMETER  :: TEN = 10.0 

double precision:: AREA, ARG, TMP ,ZPT_APPROX, ZPT_COR ,FSKY_ASEC, FSKY_PIX
...
! tack  on source/noise correction term
...
! get sky noise in photoelectrons per pixel
